...
// is not reducible. "NORTH" and "WEST", "WEST" and "SOUTH", "SOUTH" and "EAST" are not directly
...
const dirReduc = (arr) => {
    ...
    const NSReg = /NORTHSOUTH/g;
    const SNReg = /SOUTHNORTH/g;
    const WEReg = /WESTEAST/g;
    const EWReg = /EASTWEST/g;

    let str = arr.join('');

    if (str.match(NSReg) === null && str.match(SNReg) === null && str.match(WEReg) === null && str.match(EWReg) === null) return arr

    for (let i = 0; i < arr.length -1; i++) {
        if (arr[i] === 'NORTH') {
            if (arr[i + 1] === 'SOUTH') resultArr = arr.slice(0, i).concat(arr.slice(i+2))
        }
        else if (arr[i] === 'SOUTH') {
            if (arr[i+1] === 'NORTH') resultArr = arr.slice(0, i).concat(arr.slice(i+2))
        }
        else if (arr[i] === 'EAST') {
            if (arr[i+1] === 'WEST') resultArr = arr.slice(0, i).concat(arr.slice(i+2))
        }
        else {
            if (arr[i+1] === 'EAST') resultArr = arr.slice(0, i).concat(arr.slice(i+2))
        }
    }
