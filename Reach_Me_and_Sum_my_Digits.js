...
function sumDigNthTerm(initval, patternl, nthterm) {
    let arrCopy = [...patternl];
    let searchValue = initval;
    let resultArr = []

    const endIndex = patternl.length-1;
    let i = 0;

    for (i; i < nthterm; i++){
        arrCopy = [...arrCopy, ...patternl]

        searchValue = searchValue + +arrCopy[i]
        resultArr.push(searchValue)
    }

    // console.log(`searchValue = ${searchValue}`)

    resultArr = [initval, ...resultArr]
    console.log(resultArr)
    return resultArr[nthterm - 1].toString().split('').reduce((acc,el) => +acc + +el)
}
